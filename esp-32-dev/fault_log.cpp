...
void faultlog_record(const FaultEntry& e) {
  // Collapse repeats: a flapping sensor would otherwise fill the 32-slot ring in
  ...
  // Flash erase stalls the instruction cache on BOTH cores. Only commit
  // immediately when the fault is serious enough that losing it to a reset
  // would matter.
