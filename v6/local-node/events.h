...
enum EventType : uint8_t {
  ...
  // v6 wireless link (tank node <-> local node)
  EV_LINK_UP,              // payload.u32 = tank node seq at first contact
  EV_LINK_DOWN,            // payload.u32 = ms since last good frame
};
...
enum FaultCode : uint8_t {
  ...
  FC_LINK_LOST,            // v6: tank-node telemetry lost (ESP-NOW timeout)
